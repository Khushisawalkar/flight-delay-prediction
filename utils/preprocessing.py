...
# Airports known for congestion/weather delays
DELAY_PRONE_AIRPORTS = {
    "ORD": 0.35,
    "JFK": 0.30,
    "SFO": 0.28,
    "BOS": 0.32,
...
def generate_synthetic_dataset(
    ...
    # Realistic departure-hour distribution
    hour_probs = np.array([
        0.05, 0.08, 0.10, 0.10, 0.08,
    ...
    elapsed = (
        distances / 500 * 60 +
        rng.normal(0, 15, n)
    ).astype(int).clip(45, 600)

    # ── Delay Probability Modeling ────────────────────────────────────────────
    prob = np.full(n, 0.20)

    # Airline factor
    airline_reliability = np.array([
        AIRLINE_RELIABILITY[a]
        for a in airlines
    ])
    ...
    airport_congestion = np.array([
        DELAY_PRONE_AIRPORTS.get(o, 0.15)
        for o in origins
    ])
    ...
    # Peak hours
    is_peak = (
        ((dep_hours >= 7) & (dep_hours <= 9)) |
        ((dep_hours >= 17) & (dep_hours <= 20))
    )
    ...
    weather_risk = np.array([
        weather_risk_by_month.get(m, 0.05)
        for m in months
    ])
    ...
    delay_minutes[delayed_mask] = (
        rng.exponential(35, delayed_mask.sum())
        .clip(15, 300)
    )
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["carrier_delay_hist"] = df["airline"].map(
        {
            a: round(1 - r, 3)
            for a, r in AIRLINE_RELIABILITY.items()
        }
    ).fillna(0.25)
    ...
    df["airport_congestion"] = (
        df["origin"]
        .map(DELAY_PRONE_AIRPORTS)
        .fillna(0.15)
    )

    # Day of week estimate
    df["dep_dayofweek"] = (
        df["dep_day"] % 7
    ).astype(int)
...
COLUMN_ALIASES = {
    ...
    "dep_delay": "delay_minutes",
    "crs_dep_time": "dep_hour",
    "month": "dep_month",
    "day_of_month": "dep_day",
    "day_of_week": "dep_dayofweek",
...
def normalize_uploaded_csv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.dropna(
        subset=list(
            REQUIRED_COLS.intersection(set(df.columns))
        )
    )

    # Fill defaults
    df["dep_month"] = (
        df.get("dep_month", pd.Series([6] * len(df)))
        .fillna(6)
    )

    df["dep_day"] = (
        df.get("dep_day", pd.Series([15] * len(df)))
        .fillna(15)
    )

    df["dep_hour"] = (
        df.get("dep_hour", pd.Series([12] * len(df)))
        .fillna(12)
    )

    df["distance"] = (
        df.get("distance", pd.Series([800] * len(df)))
        .fillna(800)
    )

    df["scheduled_elapsed"] = (
        df.get("scheduled_elapsed", pd.Series([120] * len(df)))
        .fillna(120)
    )

    df["delay_minutes"] = (
        df.get("delay_minutes", pd.Series([0] * len(df)))
        .fillna(0)
    )
